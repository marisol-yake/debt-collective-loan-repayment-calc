...
def configure_input_sidebar() -> None:
    with st.sidebar:
        st.number_input("Servicer Monthly Payment Estimate ($):", value=0.0, step=1, key="servicer_estimate")
        st.number_input("Total Loan Balance ($):", value=0.0, step=1, key="total_balance")
        st.number_input("Annual Interest Rate (%):", value=0.0, step=1, key="annual_interest_rate")
        st.number_input("Adjusted Gross Income (AGI) ($):", value=0.0, step=1, key="agi")
        st.number_input("Household Size (For IBR, PAYE, and ICR):", value=0, step=1, key="household_size")
